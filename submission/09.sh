...
# WRITE YOUR SOLUTION BELOW:
NUM_INPUTS=$(echo "$DECODED_TX" |  jq '.vin | length ')

check_cmd "Input counting" "NUM_INPUTS" "$NUM_INPUTS"

NUM_OUTPUTS=$(echo "$DECODED_TX" |  jq '.vout | length ')
...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(echo "$DECODED_TX" |  jq  '.vout[0].value * 100000000 | floor')
...
# WRITE YOUR SOLUTION BELOW:
UTXO_TXID=$TXID
UTXO_VOUT_INDEX=$(echo "$DECODED_TX" |  jq  '.vout[0].n')
check_cmd "UTXO vout selection" "UTXO_VOUT_INDEX" "$UTXO_VOUT_INDEX"

UTXO_VALUE=$(echo "$DECODED_TX" |  jq  '.vout[0].value * 100000000 | floor')
...
CHANGE_AMOUNT=$(echo "$PAYMENT_AMOUNT - $FEE_SATS" | bc -l)
check_cmd "Change calculation" "CHANGE_AMOUNT" "$CHANGE_AMOUNT"

# Convert amounts to BTC for createrawtransaction
PAYMENT_BTC=$(echo "scale=8; $PAYMENT_AMOUNT / 100000000" | bc | xargs printf "%.8f")
CHANGE_BTC=$(echo "scale=8; $CHANGE_AMOUNT / 100000000" | bc | xargs printf "%.8f" )
...
# WRITE YOUR SOLUTION BELOW:
VERIFY_RBF=$(echo "$DECODED_TX" | jq 'any(.vin[]; .sequence < 4294967294)' )
...
PARENT_TXID=$(echo "$DECODED_TX" | jq .txid)
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(echo "$DECODED_TX" |  jq  '.vout[1].n')
...
CHILD_SEND_AMOUNT=$(echo "$DECODED_TX" |  jq -r --argjson fee "$CHILD_FEE_SATS" '.vout[1].value * 100000000 - $fee')
check_cmd "Child amount calculation" "CHILD_SEND_AMOUNT" "$CHILD_SEND_AMOUNT"

# Convert to BTC
CHILD_SEND_BTC=$(echo "scale=8; $CHILD_SEND_AMOUNT / 100000000" | bc | xargs printf "%.8f")
...
DECODED_TX=$(bitcoin-cli -regtest decoderawtransaction $SECONDARY_TX )
...
SECONDARY_OUTPUT_VALUE=$(echo "$DECODED_TX" |  jq -r   '[.vout[] | .value] | add * 100000000 ')
...
# Convert to BTC
TIMELOCK_BTC=$(echo "scale=8; $TIMELOCK_AMOUNT / 100000000" | bc | xargs printf "%.8f")
